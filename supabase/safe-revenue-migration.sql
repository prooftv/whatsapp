...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sponsors_tier_check') THEN
        ALTER TABLE sponsors ADD CONSTRAINT sponsors_tier_check CHECK (tier IN ('standard', 'premium', 'enterprise'));
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_sponsor_assets_sponsor_id ON sponsor_assets(sponsor_id);